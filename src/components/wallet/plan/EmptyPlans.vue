...
interface Mapping {
  ...
  tipologia: string;
  propertiesId: number;
  propertiesProduct_id: number;
  propertiesTipo: string;
...
const planToCompare = ref<Mapping[]>([]);
const nameFamilies = ref<Mapping[]>([]);
const boltsCredits = ref<Mapping[]>([]);
const vehicleDetail = ref<Mapping[]>([]);
const garageSetup = ref<Mapping[]>([]);
const auctionAlert = ref<Mapping[]>([]);
const socialRumors = ref<Mapping[]>([]);
const singleChart = ref<Mapping[]>([]);
const miniDashboard = ref<Mapping[]>([]);
const fullDashboard = ref<Mapping[]>([]);
const gtc = ref<Mapping[]>([]);
const searchLocate = ref<Mapping[]>([]);
const roadmap = ref<Mapping[]>([]);
const sparesParts = ref<Mapping[]>([]);
...
const buyThisPlan = (familyId: number) => {
  const family = {
    family: nameFamilies.value[0].family,
  ...
  async value => {
    if (value) {
      ...
      try {
        ...
        if (response) {
          ...
          nameFamilies.value = mappedDatas.filter(
            item =>
              ...
              item.category === 'Subscription_ppu'
          );
          boltsCredits.value = mappedDatas.filter(
            item => item.name === 'Bolts Credits'
          );
          vehicleDetail.value = mappedDatas.filter(
            item => item.name === 'Vehicle Detail'
          );
          garageSetup.value = mappedDatas.filter(
            item => item.name === 'Garage setup'
          );
          auctionAlert.value = mappedDatas.filter(
            item => item.name === 'Auction Alert'
          );
          socialRumors.value = mappedDatas.filter(
            item => item.name === 'Social Rumors'
          );
          singleChart.value = mappedDatas.filter(
            item => item.name === 'Single Chart'
          );
          miniDashboard.value = mappedDatas.filter(
            item => item.name === 'Mini Dashboard'
          );
          fullDashboard.value = mappedDatas.filter(
            item => item.name === 'Full Dashboard'
          );
          gtc.value = mappedDatas.filter(
            item => item.name === 'Gallery, text, chassis'
          );
          searchLocate.value = mappedDatas.filter(
            item => item.name === 'Search Locate'
          );
          roadmap.value = mappedDatas.filter(item => item.name === 'Roadmap');
          sparesParts.value = mappedDatas.filter(
            item => item.name === 'Spares Parts'
          );
        ...
        console.error(
          'Errore durante il recupero dei dati dei prodotti:',
          error
        );
        isLoading.value = false;
      }
