...
const brandsChart = mappedDatas.filter(item => item.name === '"Active Brand Charts"');
const maisonCharts = mappedDatas.filter(item => item.name === 'Active Maison Charts');
const auctionCharts = mappedDatas.filter(item => item.name === 'Active Auction Charts');
const indexCharts = mappedDatas.filter(item => item.name === 'Active Index Charts');
const familyCharts = mappedDatas.filter(item => item.name === 'Active Family Charts');
const modelCharts = mappedDatas.filter(item => item.name === 'Active Model Charts');
  ...
  <div class="flex items-start gap-[25px]">
    <div class="flex flex-col bg-white items-start gap-[25px]">
      ...
      <div class="flex flex-col h-[456px] items-center gap-[10px] flex-shrink-0">
        ...
        <div class="flex w-[526px] justify-center items-center" style="padding: 18px 26px 18px 25px; border-radius: 12px 12px 0px 0px; border-bottom: 1px solid rgba(33, 37, 41, 0.5); background: rgba(255, 218, 68, 0.25);">
          <div class="flex w-[475px] items-center gap-[210px] flex-shrink-0">
            <div class="flex flex-col items-start gap-[8px]" v-for="(item, index) in nameFamilies" key="index">
              <div class="text-gray-900 font-inter text-[24px] font-semibold leading-[32px]">{{ item.name }}</div>
              <div class="flex w-[161px] flex-column-reverse justify-center items-start gap-[2px]">
                <div class="text-gray-600 self-stretch font-inter text-sm font-normal leading-[20px]" v-if="item.name === 'Free-user'">Perpetual Plan</div>
                <div class="text-gray-600 self-stretch font-inter text-sm font-normal leading-[20px]" v-if="item.name === 'Pay-per-use'">Bolts required</div>
                <div class="text-gray-600 self-stretch font-inter text-sm font-normal leading-[20px]" v-if="item.name === 'Ready Plan' || item.name === 'Start Plan' ">Monthly Plan</div>
                <div class="text-gray-600 self-stretch font-inter text-sm font-normal leading-[20px]" v-if="item.name === 'Go Plan' || item.name === 'Pro Plan'">Annual Plan</div>
                <div class="flex items-start">
                  <img src="@/assets/images/bronze_token.png" alt="" style="width: 28px; height: 28px"/>
                  <div class="text-gray-900 font-inter text-[24px] font-semibold leading-[32px]">{{ item.prezzo }}</div>
                </div>
              </div>
            </div>
            <div class="flex flex-col h-[94px] items-start gap-[2px]" v-for="(item, index) in boltsCredits" key="index">
              <div class="flex flex-col items-start gap-[8px]">
                <div class="flex w-[90px] items-center gap-[4px]">
                  <img src="@/assets/images/bolt.png" alt="" class="w-[26px] h-[26px]"/>
                  <div class="text-gray-900 font-inter text-[24px] font-semibold leading-[32px]">Bolts</div>
                </div>
                <div class="text-gray-900 font-inter text-[24px] font-semibold leading-[32px]">+{{ item.prezzo }}</div>
              </div>
              <div class="text-gray-600 font-inter text-[13px] font-normal leading-[20px]">Included in plan</div>
            </div>
          </div>
        </div>
        <!-- /BADGE -->
        ...
        <div class="flex w-[510px] h-[216px] items-start gap-[6px] flex-shrink-0">
          ...
          >
            ...
            <div class="cell-container" v-for="(item, index) in socialRumors" key="index">
              <img v-if="item.propertiesValue > '-1'" src="@/assets/icons/x-circle-fill.svg" alt="" class="w-[24px] h-[24px]"/>
              <img v-if="item.propertiesValue < '-1'" src="@/assets/icons/x-circle-fill.svg" alt="" class="w-[24px] h-[24px]"/>
            </div>
          </div>
        </div>
      ...
      <div class="flex flex-col items-start gap-[16px] flex-shrink-0" style="padding: 1px 0px" >
        <div class="flex w-[526px] justify-center items-center flex-shrink-0" style="padding: 3px 0px 0px 0px" >
          <div class="flex w-[526px] h-[46px] justify-center items-center flex-shrink-0" style=" padding: 10.5px 13px 11.5px 13px; border-bottom: 1px solid rgba(33, 37, 41, 0.5); background: rgba(255, 218, 68, 0.25);">
            <div class="w-[500px] flex-shrink-0 text-[#3C3C3C] text-base font-normal" style="font-family: Montserrat">Below period usage limits </div>
          </div>
        </div>
      </div>
      <!--          MIDDLE Table-->
      ...
      <div class="flex flex-col items-end gap-[25px]">
        ...
        <div class="flex pr-[6px] items-center">
          <button class="flex text-white font-inter text-base font-normal leading-[24px] w-[230px] justify-center items-center gap-[8px] bg-[#212529]" style=" border-radius: 4px; border: 1px solid #212529; padding: 6px 12px;" @click="buyThisPlan(nameFamilies[0].family)">Buy plan</button>
        </div>
