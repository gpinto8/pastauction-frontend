  ...
  <div class="container mx-auto">
    ...
    <main class="classic-main bg-transparent">
      <div class="grid grid-cols-4 shadow">
        <div :class="'col-span-1 tab ' + (tab === 'plans' ? 'tab-selected' : '')" @click="changeTab('plans')">
          <h4 class="hidden md:block">Plans</h4>
          <img class="md:hidden invert hover:invert-0" src="@/assets/svg/documents-outline.svg"
            alt="Plans - Pastauction">
        </div>
        <div :class="'col-span-1 tab ' + (tab === 'token' ? 'tab-selected' : '')" @click="changeTab('token')">
          <h4 class="hidden md:block">Token/Cart</h4>
          <img class="md:hidden hover:invert" src="@/assets/svg/shop-window.svg" alt="Token/Cart - Pastauction">
        </div>
        <div :class="'col-span-1 tab ' + (tab === 'services' ? 'tab-selected' : '')" @click="changeTab('services')">
          <h4 class="hidden md:block">My services</h4>
          <img class="md:hidden hover:invert" src="@/assets/svg/clipboard-check.svg" alt="My services - Pastauction">
        </div>
        <div :class="'col-span-1 tab ' + (tab === 'payment' ? 'tab-selected' : '')" @click="changeTab('payment')">
          <h4 class="hidden md:block">Payment history</h4>
          <img class="md:hidden hover:invert" src="@/assets/svg/credit-card-2-back.svg"
      ...
      <div class="plans" v-if="showCarousel && tab === 'plans'">
        ...
        <PlansDetails v-else :selectedFamilyId="selectedFamilyId" @buyThisPlan="buyThisPlan" />
      </div>
