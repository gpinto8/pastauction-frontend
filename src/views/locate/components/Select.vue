...
watch(isOpen, () => {
  if (closeSelectListener.value)
    document.removeEventListener('click', closeSelectListener.value);
  if (isOpen.value) {
    setTimeout(() => {
      closeSelectListener.value = (event: Event) => {
        if (
          !selectOptionsContainerRef.value?.contains(
            event.currentTarget as Node
          )
        )
          isOpen.value = false;
      };
      document.addEventListener('click', closeSelectListener.value, {
        once: true,
      });
    }, 200);
  }
...
const filteredItems = computed(() => {
  if (props.filterFN)
    return (
      props.items?.filter(e => props.filterFN!(e, searchQuery.value)) || []
    );
  return (
    props.items?.filter(e =>
      (e + '').toLowerCase().includes(searchQuery.value.toLowerCase())
    ) || []
  );
});
  ...
  >
    <span
      ...
      :class="{
        ...
        'text-[#ADB5BD]':
          placeholderIsDisplayed && placeholderColor === '#ADB5BD',
      }"
    >
    ...
    <div
      ...
      v-if="isOpen"
      class="absolute mt-1 min-w-full w-fit top-full left-0 rounded-lg border-2 p-2 border-gray-300 flex flex-col gap-2 bg-white"
    >
      <div
        v-if="useOptionSearchFeature"
        class="flex bg-white flex-row gap-2 pb-1 border-b-[1px] items-center border-gray-300"
      >
      ...
      <div class="max-h-64 overflow-y-auto flex flex-col gap-1">
        ...
        <div
          ...
          @click.stop="
            isOpen = false;
            if (item !== selected) emit('change', item);
          "
          class="hover:bg-gray-200 flex items-center px-1 rounded-sm font-thin tracking-wide"
          :class="{ 'bg-zinc-200 cursor-default': item === selected }"
        >
          <app-icon
            v-if="item === selected"
            type="simple-check"
      ...
      <div
        ...
        @click="
          isOpen = false;
          emit('change', searchQuery as any);
          searchQuery = '';
        "
        class="text-ceter pb-1 px-1 rounded-lg bg-sky-100"
      >
        <span class="text-gray-400 font-thin">use</span>
        {{ searchQuery }}
      </div>
      <div
