...
watch(selectedBrandInitial, async () => {
  try {
    const response = await axios.get(
      `/filter/bidwatcher_brand/name_left_2/?search=name_left_1:${selectedBrandInitial.value}`
    );
    brandsCoupleLetters.value = response.data.items.map(
      (item: any) => item.left_1 as string
    );
  } catch (error) {
...
watch(selectedBrandFirstTwoLetters, async () => {
  try {
    const response = await axios.get(
      `/filter/bidwatcher_brand/name/?search=name_left_2:${selectedBrandFirstTwoLetters.value}`
    );
    brandList.value = response.data.items.map(
      (item: any) => item.name as string
    );
  } catch (error) {
    console.error('Error fetching brands:', error);
  }
    ...
    <div class="flex flex-col">
      ...
      <v-row justify="start" class="align-center mt-0">
        <v-col class="d-flex flex-wrap align-center pt-0">
          <div
            :class="{
              'd-block': selectedBrandInitial,
              'd-none': !selectedBrandInitial,
            }"
            class="mt-3"
            justify="start"
          >
            <v-btn
              ...
              class="letter-button w-10 h-10"
              :variant="
                selectedBrandFirstTwoLetters === coupleLetters
                  ? 'elevated'
                  : 'outlined'
              "
      ...
      <v-row justify="start" class="align-center mt-0">
        <v-col class="d-flex flex-wrap align-center pt-0">
          <div
            :class="{
              'd-block': selectedBrandFirstTwoLetters,
              'd-none': !selectedBrandFirstTwoLetters,
            }"
            class="mt-3 w-full"
          >
            <div class="flex flex-col">
            ...
            <div
              class="border !border-black grid grid-cols-3 lg:grid-cols-5 gap-y-2 p-2"
              text
            >
              <div
