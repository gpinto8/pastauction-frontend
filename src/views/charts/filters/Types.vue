...
const selectedFamilyName = defineModel<string | null>('selectedFamily', {
  required: true,
});
...
watch(selectedFamilyName, async () => {
  try {
    const response = await axios.get(
      '/filter/filter_charts_vehicles/body_category/',
      {
        params: {
...
const selectedCategoryName = defineModel<string | null>('selectedCategory', {
  required: true,
});
...
async function selectCategoryType(type: string, categoryType: string) {
  ...
  try {
    const response = await axios.get(
      '/filter/filter_charts_vehicles/body_shape/',
      {
        params: {
      ...
      <div class="flex flex-col">
        <div class="filters-grid-selection">
          <v-btn
            ...
            :variant="
              selectedFamilyName === type.body_type ? 'elevated' : 'outlined'
            "
        ...
        <v-row justify="start" class="align-center mt-0">
          <v-col class="d-flex flex-wrap align-center pt-0">
            <div
              :class="{
                'd-block': selectedFamilyName,
                'd-none': !selectedFamilyName,
              }"
              class="mt-3"
              justify="start"
            >
              <v-btn
                ...
                class="letter-button"
                :variant="
                  selectedCategoryName === catType.body_category
                    ? 'elevated'
                    : 'outlined'
                "
                @click="
        ...
        <v-row justify="start" class="align-center mt-0">
          <v-col class="d-flex flex-wrap align-center pt-0">
            <div
              :class="{
                'd-block': selectedCategoryName,
                'd-none': !selectedCategoryName,
              }"
              class="mt-3"
            >
              ...
              <v-row class="letter-button border-brand" color="black" text>
                ...
                >
                  <div
                    class="m-3"
                    :class="{
                      selected: types.includes(categoryName.body_shape),
                    }"
