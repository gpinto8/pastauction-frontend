...
const iconActive = computed(() =>
  internalEvent.value.disabled ? inactiveIcon : activeIcon
);
...
async function updateLogoCallback(value: any) {
  const url = await entityEventsStore.uploadLogo(
    value,
    internalEvent.value.id_key
  );
  internalEvent.value.event_logo = url;
}

async function updateMainPhotoCallback(value: any) {
  const url = await entityEventsStore.uploadMainPhoto(
    value,
    internalEvent.value.id_key
  );
    ...
    <v-container>
      <v-row class="subheader rounded-lg mb-4">
        <v-col :cols="2" class="d-flex align-center" v-if="!isAdd">
          <Linguetta
            :color="
              entityEventsStore.statusToColor(
                entityEventsStore.getStatusFromEvent(internalEvent)
              )
            "
            :width="100"
          />
      ...
      <v-row>
        <v-col :cols="4" class="d-flex">
          <v-text-field
            ...
            density="compact"
            label="ID"
            :model-value="internalEvent.id_key"
            disabled
            v-if="!isAdd"
          />
          <v-text-field
      ...
      <v-row>
        ...
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="Country"
            v-model="internalEvent.begin_country"
          />
        </v-col>
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="City"
            v-model="internalEvent.begin_city"
          />
        </v-col>
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="Address"
            v-model="internalEvent.begin_address"
          />
        </v-col>
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="Longitude"
            v-model="internalEvent.begin_lon"
          />
        </v-col>
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="Latitude"
            v-model="internalEvent.begin_lat"
          />
        </v-col>
      </v-row>
      ...
      <v-row>
        ...
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="Country"
            v-model="internalEvent.end_country"
          />
        </v-col>
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="City"
            v-model="internalEvent.end_address"
          />
        </v-col>
        <v-col>
          <v-text-field
            ...
            density="compact"
            label="Address"
            v-model="internalEvent.end_city"
          />
        </v-col>
        <v-col>
          <v-text-field
      ...
      <v-row>
        ...
        <v-col cols="12">
          <v-checkbox
            ...
            v-for="category of entityEventCategoires"
            v-model="internalEvent.event_type"
      ...
      <v-row>
        ...
        <v-col>
          <div class="d-flex align-center flex-column" style="width: 230px">
            <img
              class="mb-4"
              :src="`${apiUrl}/photo/${internalEvent.main_photo}`"
              width="230"
              v-if="internalEvent.main_photo"
            />
      ...
      <v-row>
        ...
        <v-col>
          <div class="d-flex align-center flex-column" style="width: 230px">
            <img
              class="mb-4"
              :src="`${apiUrl}/photo/${internalEvent.event_logo}`"
              width="230"
      ...
      <v-row v-if="isAdd">
        <v-col class="d-flex justify-end" :cols="12">
          <v-btn class="px-16 mr-4" color="grey" @click="resetEvent">
            Reset
          </v-btn>
