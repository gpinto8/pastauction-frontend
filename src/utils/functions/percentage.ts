 ...
 * @returns
 */
export const getPercentageFrom = (a: number, total: number) => (a / 100) * total

/**
 * Normalize the bounds of a value
 */
export const getAsPercentageBetween = (a: number, min: number, max: number) => getAsPercentage(a - min, max - min)
...
export const transposeValue: (params: TransposeValueParameters) => number = ({
  ...
  const percentageOfOriginal = getAsPercentageBetween(value, originalBounds.min, originalBounds.max)
  const transposedValue = getPercentageFrom(percentageOfOriginal, targetBounds.max - targetBounds.min)
